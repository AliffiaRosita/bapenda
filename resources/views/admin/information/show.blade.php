        ...
        <div class="modal-content">
            ...
            <div class="modal-body ">
                <div class="text-center">
                    <h3 class="mb-5">{{ $information->title }}</h3>
                </div>
                <hr>
                ...
                <div class="row ">
                    @foreach ($information->infoFiles as $key=> $file)
                    <div class="col-md-3 mb-3">{{ $key+1 }}</div>
                                <div class="col-md-3 mb-3">{{ $file->title }}</div>
                                <div class="col-md-3 mb-3">{{ $information->title }}</div>
                                <div class="col-md-3 mb-3"><a href="{{ route('information.download',['infoFile'=>$file]) }}"  class="btn btn-sm btn-success">Unduh</a></div>
                                {{-- <div class="col-md-3 mb-3"><a href="{{ asset('storage/'.$file->file) }}" target="_blank" class="btn btn-sm btn-success">Unduh</a></div> --}}
                    @endforeach
                </div>

                @elseif ($information->type == "article")
                {!! $information->infoArticle->desc !!}
