        ...
        <div class="modal-content">
            ...
            <div class="modal-body ">
                <div class="text-center">
                    <h3 class="mb-5">{{ $serviceProgram->title }}</h3>
                </div>
                <hr>
                @if ($serviceList->type == "file")
                ...
                <div class="row ">
                    @foreach ($serviceList->serviceFiles as $key=> $file)
                    <div class="col-md-3 mb-3">{{ $key+1 }}</div>
                                <div class="col-md-3 mb-3">{{ $file->title }}</div>
                                <div class="col-md-3 mb-3">{{ $serviceProgram->title }}</div>
                                <div class="col-md-3 mb-3"><a href="{{ route('serviceList.download',['serviceFile'=>$file]) }}"  class="btn btn-sm btn-success">Unduh</a></div>
                                {{-- <div class="col-md-3 mb-3"><a href="{{ asset('storage/'.$file->file) }}" target="_blank" class="btn btn-sm btn-success">Unduh</a></div> --}}
                    @endforeach
                </div>

                @elseif ($serviceList->type == "article")
                {!! $serviceList->serviceArticle->desc !!}
