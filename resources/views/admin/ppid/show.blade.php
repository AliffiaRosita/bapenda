    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body ">
                ...
                <div class="row ">
                    @foreach ($ppid->ppidFiles as $key=> $file)
                    <div class="col-md-3 mb-3">{{ $key+1 }}</div>
                                <div class="col-md-3 mb-3">{{ $file->title }}</div>
                                <div class="col-md-3 mb-3">{{ $ppid->title }}</div>
                                <div class="col-md-3 mb-3"><a href="{{ route('ppid.download',['ppidFile'=>$file]) }}"  class="btn btn-sm btn-success">Unduh</a></div>
                                {{-- <div class="col-md-3 mb-3"><a href="{{ asset('storage/'.$file->file) }}" target="_blank" class="btn btn-sm btn-success">Unduh</a></div> --}}
                    @endforeach
                </div>

                @elseif ($ppid->type == "article")
                {!! $ppid->ppidArticle->desc !!}
