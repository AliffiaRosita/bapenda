...
<div class="card mb-5 mb-xl-8">
    ...
    <div class="card-header border-0 pt-5">
        ...
        <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="klik untuk menambah data layanan PPID">
            <a href="{{ route('ppid.create') }}" class="btn btn-light-primary">
        ...
        <div class="table-responsive">
            ...
            <table id="newsTable" class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                ...
                <tbody>
                    ...
                    @foreach ($ppids as $key=> $ppid)
                    <tr>
                        <td>{{ $key+1 }}</td>
                        <td>{{ $ppid->title }}</td>
                        <td class="text-center">
                            <a href="{{ route('ppid.edit',['ppid'=>$ppid]) }}" class="btn btn-light-success btn-sm">Ubah</a>
                            <button type="button" class="btn btn-light-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail-{{ $key }}">
                                Detail
                            </button>

                            <form  method="post" class="d-inline" onsubmit="btnDelete('ppid',{{ $ppid->id}})">
                                @csrf
                                @method('delete')
